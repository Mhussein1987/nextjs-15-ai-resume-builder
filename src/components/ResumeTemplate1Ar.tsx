...
const ResumeTemplate1Ar = React.memo(function ResumeTemplate1Ar({
  resumeData,
  className,
}: ResumePreviewProps) {
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        data-template="template1ar" // Add data attribute for print CSS targeting
      >
        <div
        ...
        >
          ...
          <div className="space-y-3">
            ...
            {resumeData.userLanguages && resumeData.userLanguages.length > 0 ? (
              ...
              <p className="text-sm text-gray-500 italic">لم يتم إضافة لغات بعد</p>
            )}
          </div>
        </div>
      </div>
...
ResumeTemplate1Ar.displayName = 'ResumeTemplate1Ar';

export default ResumeTemplate1Ar;
    ...
    <>
      ...
      <div className="space-y-3">
        ...
        {workExperiencesNotEmpty && workExperiencesNotEmpty.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 italic">لم يتم إضافة خبرة عمل بعد</p>
        )}
      </div>
    </>
  );
...
const WorkExperienceItem = React.memo(function WorkExperienceItem({ 
  ...
  const dateRange = React.useMemo(() => {
    ...
    return `${formatDate(experience.startDate, "MM/yyyy")} - ${
      experience.endDate ? formatDate(experience.endDate, "MM/yyyy") : "الحالي"
    }`;
  ...
  return (
    <>
      ...
      <div className="space-y-3">
        ...
        {educationsNotEmpty && educationsNotEmpty.length > 0 ? (
          educationsNotEmpty.map((edu, index) => (
            <EducationItem key={index} education={edu} />
          ))
        ) : (
          <p className="text-sm text-gray-500 italic">لم يتم إضافة تعليم بعد</p>
        )}
      </div>
    </>
  );
